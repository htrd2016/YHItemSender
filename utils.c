...
int send_data(const char *server_ip, int server_port, const char *agent_host_name,
            char (*item_key)[512], char (*data)[64], int count)
{
  int index = 0;
  for (index=0; index<count; index++)
  {
    //sprintf(cmd, "zabbix_sender -z %s -p %d -s \"%s\" -k %s -o %s", server_ip, server_port, agent_host_name, item_key[index], data[index]);
    //printf("%s\n", cmd);
    send_one_data(server_ip, server_port, agent_host_name, item_key[index], data[index]);
  }
  return 0;
}
